...
if ckpt:
    print('Did not restore ' + ckpt.model_checkpoint_path)
...
noisy_images = {}
noisy_images['300'] = [None] * len(train_ids)
noisy_images['250'] = [None] * len(train_ids)
noisy_images['100'] = [None] * len(train_ids)
...
for epoch in range(lastepoch, 226):
    ...
    for ind in np.random.permutation(len(train_ids)):
        ...
        gt_patch = noisy_image[:, yy * 2:yy * 2 + ps * 2, xx * 2:xx * 2 + ps * 2, :]
    ...
    if(epoch % 10 == 0):
      saver.save(sess, drive_checkpoint_dir + 'model.ckpt')
